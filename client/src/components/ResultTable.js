...
export const ResultTable = ({tableName, fieldNames, data, deletable}) => {
    ...
    return (
        <Table responsive>
            ...
            <tbody>
                {data.map((entry, idx) => <tr key={idx} >{entry.filter((el, i) => i != id_index).map((att, indx) => <td key={indx}>{att}</td>)} {deletable && <DeleteButton tableName={tableName} id={entry[id_index]}/>} </tr>)}
            </tbody>
        </Table>
    );
};
